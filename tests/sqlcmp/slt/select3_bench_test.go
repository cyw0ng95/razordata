...
func setupSelect3(b *testing.B) *sql.DB {
	b.Helper()
...
// BenchmarkSelect3_Queries runs representative select3 queries through
// the full engine path (database/sql → driver → executor → storage).
// select3 tests CASE expressions with subqueries and NULL-heavy data.
func BenchmarkSelect3_Queries(b *testing.B) {
	db := setupSelect3(b)
	...
	queries := []struct {
		...
		{"case_subquery", "SELECT CASE WHEN c>(SELECT avg(c) FROM t1) THEN a*2 ELSE b*10 END FROM t1"},
		{"case_simple", "SELECT CASE WHEN a<b-3 THEN 111 WHEN a<=b THEN 222 WHEN a<b+3 THEN 333 ELSE 444 END FROM t1"},
		...
		{"groupby", "SELECT b, count(*) FROM t1 GROUP BY b"},
	}
...
// BenchmarkSelect3_Prepared uses prepared statements to measure
// overhead without parse time.
func BenchmarkSelect3_Prepared(b *testing.B) {
	db := setupSelect3(b)
	ctx := context.Background()
...
// BenchmarkSelect3_Throughput measures total queries/second.
func BenchmarkSelect3_Throughput(b *testing.B) {
	db := setupSelect3(b)
	ctx := context.Background()
