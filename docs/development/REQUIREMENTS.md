...
| REQ000454 | SQL/EX | JOIN duplicate rows — queries return exactly 2x the expected row count (e.g., 60 cells vs 30 expected in select1, 120 vs 60 in select2). Pattern: `hash: got N*2 cells, want N`. Discovered via `TestSLT_Each/select1.test` (L94, L101) and select3 — suggests INNER JOIN produces duplicate rows from Cartesian product, likely due to duplicate table registration or missing join condition application | critical | M | iter-27 | `SQL/EX/join.go` — NestedLoopJoin or `SQL/EX/source.go` — table registration path |
| REQ000455 | SQL/EX | Subquery planner can't see tables — `SELECT CASE WHEN c>(SELECT avg(c) FROM t1) THEN a*2 ELSE b*10 END` fails with "result mismatch" across select1 (961 fails), select2 (833 fails), select3 (2974 fails). `evalExists`/`evalInSubquery`/`evalScalarSubquery` use `NewPlanner()` with no store; subqueries cannot see tables. Discovered via `TestSLT_Each/select*.test` — SELECT without FROM may return 0 rows, further compounding issue | critical | XL | iter-27 | `SQL/EX/eval.go` — `evalScalarSubquery`, `evalExists`, `evalInSubquery`; needs store threading through eval context |
...
| REQ000457 | SQL/EX | Package-level EX state incomplete cleanup — `UnregisterAll()` (source.go:L71) does NOT clear `triggerReg` and `tableTriggers` (source.go:L18-23). Also `currentCatalog` singleton may persist across engine Close/Open cycles. Causes "ex: table already exists" on first CREATE TABLE even in isolated test runs, polluting subsequent test runs with stale in-memory rows | high | S | iter-28 | `SQL/EX/source.go` `UnregisterAll()` — add `triggerReg = map[string]*PS.TriggerStmt{}` and `tableTriggers = map[string][]*PS.TriggerStmt{}` cleanup |
| REQ000458 | SQL/EX | Result shape mismatch — queries produce wrong cell counts: 60 vs 30, 120 vs 60, 0 vs 80. Some queries return 0 rows when results are expected (e.g., `SELECT ... WHERE d NOT BETWEEN 110 AND 150` returns 0 rows, want 80). Discovered via `TestSLT_Each/select*.test` — likely compound bug involving JOIN duplicates + subquery failures + NULL semantics | critical | M | iter-27 | `SQL/EX/` — multiple paths; likely JOIN (454) + subquery (455) + NULL comparison (459) |
| REQ000459 | SQL/EX | NULL comparison returns wrong result — `SELECT 1 IN (NULL)` and `SELECT NULL IN (1)` fail. NULL three-valued logic not properly applied in IN expressions. `WHERE d NOT BETWEEN 110 AND 150` returns 0 rows — likely NULL values in column cause entire expression to evaluate to NULL (UNKNOWN) which filters out rows. Discovered via `TestSLT_Each/in1.test` (42 NULL-related in1 failures) and select queries with NULL handling in data | high | M | iter-28 | `SQL/EX/eval.go` — NULL comparison paths; check that NULL comparisons return NULL (not boolean false) consistently |
...
| REQ000474 | SQL/EX | `UNION` / `INTERSECT` / `EXCEPT` compound queries — produce wrong results or fail. Discovered via `TestSLT_Each/index/` subset tests | medium | S | iter-28 | `SQL/EX/ex.go` — compound query operators |
...
| REQ000477 | SQL/EX | `INSERT` with `RETURNING` clause — `INSERT INTO t VALUES (...) RETURNING *` or `RETURNING col` returns 0 rows or fails. Discovered via corpus tests | medium | S | iter-28 | `SQL/EX/writers.go` — CreateTable/Insert RETURNING support |
...
| REQ000305 | TXN/MV | Generational arena with old generation epoch-reclaim — lazy old allocation on first promotion (1 MB, guarded by `initOldMu`); `PutArena` moves promoted old to global `pendingOlds` list instead of retaining in arena pool; `ReclaimOldGenerations()` drains list, called by LC epoch manager on each 100ms tick; reduces epoch manager pressure by reclaiming old generations asynchronously | iter-28 |
| REQ000444 | SQL/EX | UPDATE deadlock fix — `numericFloat` in `eval.go` didn't handle Go `int` type from `?` placeholders, causing UPDATE expressions (like `x + 1`) to return nil; `equalValue` and `Eval(*PS.Param)` normalized to return `int64`; store UPDATE path now captures `oldRow` via `cloneRow` before `applyUpdate` and calls `maintainIndexesOnUpdate` after store insert | iter-28 |
| REQ000451 | SQL/PS | `CREATE TEMP VIEW` parser — `parseCreateView` now accepts optional `TEMP`/`TEMPORARY` keyword between `CREATE` and `VIEW`; `CreateViewStmt.Temporary` field set accordingly; added `T_TEMP`/`T_TEMPORARY` tokens | iter-28 |
| REQ000452 | SQL/PS | `INSERT OR REPLACE` / `REPLACE INTO` parser — `parseInsert` handles `INSERT OR ROLLBACK/ABORT/FAIL/IGNORE/REPLACE` via lexeme comparison (not hard keywords); `parseReplace` handles standalone `REPLACE INTO`; added `ConflictAction` type and field to `Insert` AST | iter-28 |