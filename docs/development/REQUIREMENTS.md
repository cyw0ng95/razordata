...
| REQ000461 | SQL/PS | Window function OFFSET in LAG/LEAD — `LAG(x, N)` and `LEAD(x, N)` with arbitrary positive integer offset not supported. Discovered via `TestSLT_Each` corpus subset | low | S | iter-28 | `SQL/EX/window.go` — window function argument parsing |
| REQ000462 | SQL/EX | Scalar function `typeof(X)` — returns string type name (`"integer"`, `"text"`, `"real"`, `"blob"`, `"null"`). Not implemented in `evalFunction`. Discovered via `TestSLT_Each` corpus — classified as "not supported" by RazorClassifier and skipped | low | S | iter-28 | `SQL/EX/eval.go` `evalFunction` — add `typeof` case |
...
| REQ000473 | SQL/PS | `PRAGMA` parser support — `PRAGMA <name> = <value>` and `PRAGMA <name>` syntax not parsed. Discovered via corpus tests that exercise pragmas | low | S | iter-28 | `SQL/PS/ps.go` — add PRAGMA statement parser |
| REQ000474 | SQL/EX | `UNION` / `INTERSECT` / `EXCEPT` compound queries — produce wrong results or fail. Discovered via `TestSLT_Each/index/` subset tests | medium | S | iter-27 | `SQL/EX/ex.go` — compound query operators |
...
| REQ000476 | SQL/EX | `TRUNCATE TABLE` — not implemented, fails with parser error. Discovered via corpus tests | low | S | iter-28 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — add TRUNCATE parsing and execution |
...
| REQ000478 | SQL/EX | `REINDEX` — not implemented, fails with "feature not supported". Discovered via `TestSLT_Each/slt_lang_reindex.test` | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp REINDEX routing |
| REQ000479 | SQL/PS | `CREATE INDEX IF NOT EXISTS` — parser doesn't accept `IF NOT EXISTS` clause. Discovered via corpus tests | low | S | iter-28 | `SQL/PS/ps.go` — CREATE INDEX parser |
| REQ000480 | SQL/EX | `DROP INDEX` — not implemented. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp DROP INDEX routing |
| REQ000481 | SQL/EX | `EXPLAIN` statement — `EXPLAIN SELECT ...` returns wrong format or fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — EXPLAIN execution path |
| REQ000482 | SQL/EX | `AUTOINCREMENT` keyword — `CREATE TABLE t (id INTEGER PRIMARY KEY AUTOINCREMENT)` parser accepts but autoincrement behavior (monotonic rowid) not implemented | low | S | iter-28 | `SQL/EX/writers.go` — CreateTable autoincrement handling |
...
| REQ000487 | SQL/EX | `VACUUM` not routed to executor — `VACUUM` statement fails with "feature not supported" or "syntax error". Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp VACUUM routing |
| REQ000488 | SQL/EX | `ANALYZE` not routed to executor — `ANALYZE` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp ANALYZE routing |
...
| REQ000490 | SQL/EX | `PRAGMA` not routed to executor — `PRAGMA` statements fail. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp PRAGMA routing |
...
| REQ000492 | SQL/EX | `CREATE INDEX` not routed to executor — `CREATE INDEX` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp CREATE INDEX routing |
| REQ000493 | SQL/EX | `CREATE VIEW` not routed to executor — `CREATE VIEW` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp CREATE VIEW routing |
| REQ000494 | SQL/EX | `DROP VIEW` not routed to executor — `DROP VIEW` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp DROP VIEW routing |
| REQ000495 | SQL/EX | `CREATE TRIGGER` not routed to executor — `CREATE TRIGGER` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp CREATE TRIGGER routing |
| REQ000496 | SQL/EX | `DROP TRIGGER` not routed to executor — `DROP TRIGGER` statement fails. Discovered via corpus tests | low | S | iter-28 | `SQL/EX/ex.go` — buildWriterOp DROP TRIGGER routing |
| REQ000497 | SQL/EX | `DROP TABLE IF EXISTS` — parser doesn't accept `IF EXISTS` clause on DROP TABLE. Discovered via corpus tests | low | S | iter-28 | `SQL/PS/ps.go` — DROP TABLE parser |
| REQ000498 | SQL/EX | `ALTER TABLE` RENAME COLUMN — `ALTER TABLE t RENAME COLUMN a TO b` not supported. Discovered via corpus tests | low | S | iter-28 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — ALTER TABLE RENAME parsing |
...
| REQ000500 | SQL/EX | `EXPLAIN QUERY PLAN` — not supported. Discovered via corpus tests | low | S | iter-28 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — EXPLAIN QUERY PLAN parsing and execution |
...
| REQ000505 | SQL/PS | `CREATE INDEX ON ...` inside view test — `CREATE INDEX t1i1 ON t1(x)` after `CREATE VIEW` returns "catalog: table already exists: index". Duplicate index name not handled. Discovered via `slt_lang_createview.test` L15 | low | S | iter-28 | `SQL/EX/writers.go` `CreateIndex` + `ENG/LS/catalog.go` `PutIndex` — idempotent index creation |
| REQ000506 | SQL/EX | `evalInSubquery` infinite recursion — correlated `IN (SELECT ...)` subqueries trigger deep recursive `evalBinary` chain: `evalIn` → `evalInSubquery` → `runSubqueryPlan` → `Filter.Next` → `Eval` → `evalBinary` loops back to `evalIn`. Stack shows 20+ nested `evalBinary` → `Eval` → `evalBinary` frames. Causes timeout on corpus files with nested subquery-IN-where patterns (e.g. `index/orderby` files). Distinct from REQ000501 (mergeIterator hang) | critical | L | iter-28 | `SQL/EX/eval.go` `evalInSubquery`, `evalBinary` — subquery re-evaluation must cache or short-circuit to prevent O(N^N) recursion |
| REQ000507 | SQL/EX | `ROW_NUMBER()` window function returns constant — `SELECT id, ROW_NUMBER() OVER (ORDER BY id) FROM t` returns `[3,3], [3,3], [3,3]` instead of `[1,1], [2,2], [3,3]`. All rows get same rank. `computeRank` or window materialization broken. Discovered via `TestEdge_Expressions` | high | M | iter-28 | `SQL/EX/window.go` `computeRank` / `nextWindowRow` — rank counter not advancing per row |
| REQ000508 | SQL/EX | SELECT returns half expected rows — `SELECT ... WHERE x IN (SELECT ...)` returns `got 4, want 8`. Queries with subquery IN in WHERE clause produce half the expected result set. Distinct from REQ000502 (2x duplication) — this is the opposite direction. Discovered via `TestSLT_Each/select2.test` Lfirst-failure | high | M | iter-28 | `SQL/EX/eval.go` `evalInSubquery` — subquery result set not properly joined with outer rows |
| REQ000509 | SQL/EX | DELETE with WHERE on VIEW returns wrong count — `slt_lang_droptable.test`: DELETE after DROP of view fails with unexpected row count. `ex: table not registered for storage: view1` on DDL after DROP VIEW. View state not cleaned up from EX registry. Discovered via `TestSLT_Each/slt_lang_createview.test` L73 | medium | S | iter-28 | `SQL/EX/writers.go` `DropView` + `source.go` `UnregisterAllViews` — view not removed from table registry on DROP |
| REQ000510 | SQL/EX | `UPDATE` on table with index hangs — first UPDATE in `slt_lang_update.test` hangs with deadline exceeded after INSERT+CREATE INDEX. Stack: `Update.nextFromStore` → `SeqScan` → `mergeIterator` → `memtableIter`. The UPDATE read path re-enters the same mergeIterator hang as REQ000501 but specifically triggered by UPDATE+index combination | critical | L | iter-28 | `SQL/EX/writers.go` `Update.nextFromStore` — shares root cause with REQ000501; fix mergeIterator to unblock UPDATE path |
| REQ000511 | SQL/EX | `INSERT ... ON CONFLICT DO UPDATE` not implemented — `writers.go:120-122` comment confirms "For now, just continue (full implementation would update existing row)". Falls through to next row on conflict instead of updating the conflicting row | high | M | iter-28 | `SQL/EX/writers.go:120-122` `Insert.Next` — implement actual update of conflicting row when `onConflict.DoUpdate` set |
| REQ000512 | SQL/EX | INSERT/UPDATE/DELETE RETURNING returns only first row — `writers.go:154-158` returns `i.resultRows[0]` and sets `i.resultPos = 1` but `Query` callers see only one row. Multi-row RETURNING results are silently dropped | high | S | iter-28 | `SQL/EX/writers.go` `Insert.Next`, `Update.Next`, `Delete.Next` — return full `resultRows` iterator, not just first |
| REQ000513 | SQL/EX | FK validation missing on UPDATE — `validateForeignKeyInsert` called on INSERT but no `validateForeignKeyUpdate` on UPDATE path. `writers.go:486-504` (Update path) doesn't check FK constraints | high | M | iter-28 | `SQL/EX/fk.go` — add `validateForeignKeyUpdate` and wire into `Update.Next` |
| REQ000514 | SQL/EX | FK validation missing on DELETE — `validateForeignKeyDelete` exists but is not wired into the DELETE executor path. `writers.go:609-641` calls no FK validator | high | M | iter-28 | `SQL/EX/writers.go` `Delete.Next` — call `validateForeignKeyDelete` before delete |
| REQ000515 | SQL/EX | `fillDefaults` may not apply DEFAULT for BOOLEAN/INT/TEXT columns when INSERT omits them — only `cschema.defaults[i]` is checked; column type coercion path is missing for omitted columns | medium | S | iter-28 | `SQL/EX/writers.go` `fillDefaults` — apply DEFAULT for BOOLEAN/INT/TEXT when INSERT omits column |
| REQ000516 | SQL/EX | `validateCheck` not applied to UPDATE — CHECK constraints enforced on INSERT (`writers.go:108`) but missing on UPDATE write path (`writers.go:486-504`) | medium | S | iter-28 | `SQL/EX/constraints.go` `validateCheck` — call from `Update.Next` |
| REQ000517 | SQL/EX | `checkUnique` not applied on UPDATE — UNIQUE constraints enforced on INSERT (`writers.go:111`) but missing from UPDATE | medium | M | iter-28 | `SQL/EX/constraints.go` `checkUnique` — call from `Update.Next` |
| REQ000518 | SQL/EX | `RETURNING *` (all columns) not supported — `parseReturning` only accepts explicit column list, no `*` expansion. SLT corpus `select4.test` uses `INSERT ... RETURNING *` | medium | S | iter-28 | `SQL/PS/ps.go` `parseReturning` — accept `*` and expand to all table columns |
...
| REQ000520 | SQL/PS | `CREATE TABLE AS SELECT` not supported — no parser path, no `CreateTableAsStmt` AST node. SLT corpus extensively uses `CREATE TABLE ... AS SELECT ...` | medium | M | iter-28 | `SQL/PS/ps.go` + `SQL/EX/writers.go` — add parser path, AST node, executor; infer column types from SELECT result schema |
...
| REQ000525 | SQL/EX | Correlated subquery in SELECT list returns 0 rows — `slt_gap_test.go:30-33` `correlated_subquery` probe. `(SELECT count(*) FROM t1 AS x WHERE x.b<t1.b)` not re-evaluated per outer row | critical | L | iter-28 | `SQL/EX/eval.go` `evalScalarSubquery` — re-evaluate per outer row, threading outer row context |
...
| REQ000532 | TXN/MV | `Arena.promote()` data race on lazy old-gen allocation — `a.old == nil` is read at `arena.go:210` *before* acquiring `initOldMu`; two concurrent `promote()` calls can both observe `nil`, race into the lock, and both allocate (the second clobbers the first). Race detector flags `promote()` line 210 on `go test -race`. REQ000305's lazy-allocation design is correct but the read-outside-lock pattern is unsound | high | S | REQ000305, iter-28 (audit) | `TXN/MV/arena.go` `promote()` — always acquire `initOldMu` before the `a.old == nil` check |
...
| REQ000049 | ENG/SC | Schema cluster split from LS (TableSchema, ColumnDef, ColumnType, Row, Validator) | iter-27 |
| REQ000050 | ENG/DP | Deparser cluster split from LS (EncodeRow/DecodeRow, EncodeBlock/DecodeBlock) | iter-27 |
| REQ000300 | ENG/LS | Tier-aware storage scheduler (PlacementPolicy, StoragePolicy, per-level device routing) | iter-27 |
| REQ000302 | MEM/BF | PMem-aware buffer pool (MADV_HUGEPAGE, PMemFile, slot tier field) | iter-27 |
...
| REQ000048 | ENG | Table registry persistence (`ENG/TB/`) — new `internal/ENG/TB/` package with `Registry` (in-memory table registry) and `Catalog` (persistent on-disk catalog.dat); atomic temp-file rename for crash safety; `ENG/LS/table.go` now delegates to `ENG/TB` | iter-27 |
