...
| REQ000454 | SQL/PL | JOIN duplicate rows — `PL/memo.go` `writeStmt` missing `Joins`, `GroupBy`, `Having`, `SubqueryFrom` fields in `Select` serialization; caused different join queries (INNER ON vs CROSS) to share the same plan memo key, reusing stale ON-filtered plan; added missing field serialization | iter-28 |