...
| REQ000456 | SYS | CREATE TABLE hangs with "deadline exceeded" on select4.test, select5.test, slt_lang_update.test — first CREATE TABLE in these files fails with `razordata: deadline exceeded` (30s timeout). Engine appears to deadlock during catalog write path or LSM initialization. Stack trace from corpus test showed goroutine blocked in `skiplist.Insert` during `Update.nextFromStore`. Affects basic DDL in fresh engines | critical | L | iter-27 | `ENG/LS/` — skiplist insert path, `catalog.go:Put` flush; check for goroutine contention on catalog lock or LSM mutex |
...
| REQ000460 | SQL/EX | UPDATE executor produces wrong results — 19 fails in `slt_lang_update.test`. First 5 statements (CREATE TABLE, 3 INSERTs, CREATE INDEX) hang with "deadline exceeded". Remaining UPDATE queries fail with result mismatch. Discovered via `TestSLT_Each/slt_lang_update.test` | critical | M | iter-27 | `SQL/EX/writers.go` — Update.Next / Update.nextFromStore; DDL hang + UPDATE execution |
...
| REQ000457 | SQL/EX | Package-level EX state complete cleanup — `UnregisterAll()` clears `triggerReg` and `tableTriggers` maps | iter-27 |
