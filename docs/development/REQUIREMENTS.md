...
| REQ000461 | SQL/PS | Window function OFFSET in LAG/LEAD — `LAG(x, N)` and `LEAD(x, N)` with arbitrary positive integer offset not supported. Discovered via `TestSLT_Each` corpus subset | low | S | iter-27 | `SQL/EX/window.go` — window function argument parsing |
| REQ000462 | SQL/EX | Scalar function `typeof(X)` — returns string type name (`"integer"`, `"text"`, `"real"`, `"blob"`, `"null"`). Not implemented in `evalFunction`. Discovered via `TestSLT_Each` corpus — classified as "not supported" by RazorClassifier and skipped | low | S | iter-27 | `SQL/EX/eval.go` `evalFunction` — add `typeof` case |
| REQ000463 | SQL/EX | `PRAGMA` statements not supported — `PRAGMA journal_mode`, `PRAGMA synchronous`, `PRAGMA cache_size` fail with "feature not supported" or "pragma not supported". Discovered via `TestSLT_Each` corpus. Already has REQ000242 for pragmas in SYS but executor doesn't route them | low | S | iter-27 | `SQL/EX/ex.go` — PRAGMA statement routing in buildWriterOp |
| REQ000464 | SQL/EX | `LIMIT` / `OFFSET` not supported — `SELECT ... LIMIT n OFFSET m` fails with parser error or wrong results. Discovered via `TestSLT_Each` corpus subset — evidence tests and select tests use LIMIT/OFFSET extensively | medium | S | iter-27 | `SQL/EX/ex.go` — LIMIT/OFFSET operator in pipeline |
...
| REQ000466 | SQL/EX | `CASE WHEN` with complex expressions — `CASE a+1 WHEN b THEN 111 WHEN c THEN 222 WHEN d THEN 333 WHEN e THEN 444 ELSE 555 END` fails. Discovered via `TestSLT_Each/select4.test` (L124). Simple CASE with expression arms not supported | medium | S | iter-27 | `SQL/EX/eval.go` — `evalCase` expression arms evaluation |
| REQ000467 | SQL/EX | `GROUP_CONCAT` with DISTINCT and ORDER BY — `GROUP_CONCAT(DISTINCT x)` produces wrong results for multi-column DISTINCT. Empty GROUP_CONCAT on non-empty groups returns NULL instead of empty string. Discovered via `TestSLT_Each/slt_lang_aggfunc.test` and select tests with GROUP BY | medium | M | iter-27 | `SQL/EX/eval.go` — `evalAggregate` DISTINCT path |
| REQ000468 | SQL/EX | `HAVING` clause not applied correctly — queries with HAVING filter produce all rows (no filtering) or 0 rows. Discovered via `TestSLT_Each/select*.test` — many queries combine GROUP BY + HAVING | medium | S | iter-27 | `SQL/EX/ex.go` — HAVING filter application after aggregation |
| REQ000469 | SQL/EX | `DISTINCT` on non-primary-key columns — produces duplicate rows when DISTINCT is applied to columns that aren't part of the primary key. Discovered via `TestSLT_Each/select*.test` | medium | S | iter-27 | `SQL/EX/ex.go` — Distinct operator |
| REQ000470 | SQL/EX | `ORDER BY` with expressions — `ORDER BY a+b*2+c*3` not supported. Simple column ORDER BY works but ORDER BY expressions fail. Discovered via `TestSLT_Each/select1.test` (L101), select2.test, select3.test | medium | S | iter-27 | `SQL/EX/ex.go` — Sort operator expression evaluation |
| REQ000471 | SQL/EX | `CAST` to various types — `CAST(x AS INTEGER)`, `CAST(x AS REAL)`, `CAST(x AS TEXT)`, `CAST(x AS BLOB)` not properly implemented. Discovered via `TestSLT_Each` cast test files | medium | S | iter-27 | `SQL/EX/eval.go` — `evalCast` or type affinity in projection |
| REQ000472 | SQL/EX | `COALESCE` with many arguments — `COALESCE(a, b, c, ...)` with more than 2 arguments returns wrong result or panics. Discovered via `TestSLT_Each` corpus subset | medium | S | iter-27 | `SQL/EX/eval.go` — `evalCoalesce` variadic handling |
| REQ000473 | SQL/PS | `PRAGMA` parser support — `PRAGMA <name> = <value>` and `PRAGMA <name>` syntax not parsed. Discovered via corpus tests that exercise pragmas | low | S | iter-27 | `SQL/PS/ps.go` — add PRAGMA statement parser |
...
| REQ000475 | SQL/EX | `DELETE` with `ORDER BY` / `LIMIT` — `DELETE FROM t WHERE ... ORDER BY ... LIMIT ...` not supported. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/writers.go` — Delete operator |
| REQ000476 | SQL/EX | `TRUNCATE TABLE` — not implemented, fails with parser error. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — add TRUNCATE parsing and execution |
...
| REQ000478 | SQL/EX | `REINDEX` — not implemented, fails with "feature not supported". Discovered via `TestSLT_Each/slt_lang_reindex.test` | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp REINDEX routing |
| REQ000479 | SQL/PS | `CREATE INDEX IF NOT EXISTS` — parser doesn't accept `IF NOT EXISTS` clause. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` — CREATE INDEX parser |
| REQ000480 | SQL/EX | `DROP INDEX` — not implemented. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp DROP INDEX routing |
| REQ000481 | SQL/EX | `EXPLAIN` statement — `EXPLAIN SELECT ...` returns wrong format or fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — EXPLAIN execution path |
| REQ000482 | SQL/EX | `AUTOINCREMENT` keyword — `CREATE TABLE t (id INTEGER PRIMARY KEY AUTOINCREMENT)` parser accepts but autoincrement behavior (monotonic rowid) not implemented | low | S | iter-27 | `SQL/EX/writers.go` — CreateTable autoincrement handling |
| REQ000483 | SQL/EX | `DEFAULT` values — columns with `DEFAULT` values not properly applied on INSERT when column is omitted. Discovered via `TestSLT_Each` corpus subset | medium | S | iter-27 | `SQL/EX/writers.go` — Insert operator DEFAULT handling |
| REQ000484 | SQL/EX | `CHECK` constraint enforcement — `CHECK` constraints on columns not enforced during INSERT/UPDATE. Discovered via corpus tests | medium | S | iter-27 | `SQL/EX/writers.go` — Insert/Update constraint validation |
| REQ000485 | SQL/EX | `UNIQUE` constraint enforcement — `UNIQUE` constraints on columns not enforced during INSERT. Discovered via corpus tests | medium | S | iter-27 | `SQL/EX/writers.go` — Insert unique constraint check |
| REQ000486 | SQL/EX | `ON CONFLICT` conflict resolution — `INSERT OR IGNORE`, `INSERT OR ROLLBACK`, `INSERT OR ABORT` not implemented. Only `INSERT OR REPLACE` partially supported. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/writers.go` — Insert conflict resolution |
| REQ000487 | SQL/EX | `VACUUM` not routed to executor — `VACUUM` statement fails with "feature not supported" or "syntax error". Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp VACUUM routing |
| REQ000488 | SQL/EX | `ANALYZE` not routed to executor — `ANALYZE` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp ANALYZE routing |
| REQ000489 | SQL/EX | `REINDEX` not routed to executor — `REINDEX` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp REINDEX routing |
| REQ000490 | SQL/EX | `PRAGMA` not routed to executor — `PRAGMA` statements fail. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp PRAGMA routing |
| REQ000491 | SQL/EX | `DROP INDEX` not routed to executor — `DROP INDEX` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp DROP INDEX routing |
| REQ000492 | SQL/EX | `CREATE INDEX` not routed to executor — `CREATE INDEX` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp CREATE INDEX routing |
| REQ000493 | SQL/EX | `CREATE VIEW` not routed to executor — `CREATE VIEW` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp CREATE VIEW routing |
| REQ000494 | SQL/EX | `DROP VIEW` not routed to executor — `DROP VIEW` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp DROP VIEW routing |
| REQ000495 | SQL/EX | `CREATE TRIGGER` not routed to executor — `CREATE TRIGGER` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp CREATE TRIGGER routing |
| REQ000496 | SQL/EX | `DROP TRIGGER` not routed to executor — `DROP TRIGGER` statement fails. Discovered via corpus tests | low | S | iter-27 | `SQL/EX/ex.go` — buildWriterOp DROP TRIGGER routing |
| REQ000497 | SQL/EX | `DROP TABLE IF EXISTS` — parser doesn't accept `IF EXISTS` clause on DROP TABLE. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` — DROP TABLE parser |
| REQ000498 | SQL/EX | `ALTER TABLE` RENAME COLUMN — `ALTER TABLE t RENAME COLUMN a TO b` not supported. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — ALTER TABLE RENAME parsing |
| REQ000499 | SQL/EX | `ALTER TABLE` DROP COLUMN — `ALTER TABLE t DROP COLUMN c` not supported. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — ALTER TABLE DROP parsing |
| REQ000500 | SQL/EX | `EXPLAIN QUERY PLAN` — not supported. Discovered via corpus tests | low | S | iter-27 | `SQL/PS/ps.go` + `SQL/EX/ex.go` — EXPLAIN QUERY PLAN parsing and execution |
...
| REQ000503 | SQL/EX | Scalar `IN (literal-list)` returns 0 rows — `SELECT 1 IN (2)` and `SELECT 1 NOT IN (2)` both return 0 rows instead of 1 row with 0/1. `evalIn` path doesn't produce a result row for scalar IN expressions outside FROM clause. in1.test: 53/310 fail. in2.test: 9/80 fail (95 skipped). Existing REQ000446 tracks this but confirms severity from SLT run | medium | M | iter-27 | `SQL/EX/eval.go` `evalIn`, `evalNotIn` — scalar IN must produce one-row result |
| REQ000504 | SQL/EX | DELETE executor wrong row count — `slt_lang_droptable.test`: 2 failures out of 14 records. DROP TABLE succeeds but DELETE with WHERE may return wrong affected-row count. Discovered via corpus per-file test | medium | S | iter-27 | `SQL/EX/writers.go` `Delete.Next` — affected row count |
| REQ000505 | SQL/PS | `CREATE INDEX ON ...` inside view test — `CREATE INDEX t1i1 ON t1(x)` after `CREATE VIEW` returns "catalog: table already exists: index". Duplicate index name not handled. Discovered via `slt_lang_createview.test` L15 | low | S | iter-27 | `SQL/EX/writers.go` `CreateIndex` + `ENG/LS/catalog.go` `PutIndex` — idempotent index creation |
...
| REQ000509 | SQL/EX | DELETE with WHERE on VIEW returns wrong count — `slt_lang_droptable.test`: DELETE after DROP of view fails with unexpected row count. `ex: table not registered for storage: view1` on DDL after DROP VIEW. View state not cleaned up from EX registry. Discovered via `TestSLT_Each/slt_lang_createview.test` L73 | medium | S | iter-27 | `SQL/EX/writers.go` `DropView` + `source.go` `UnregisterAllViews` — view not removed from table registry on DROP |
