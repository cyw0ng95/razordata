...
| 28 | Parser DDL Hardening + SQL Compliance + MV-OCC | 6 phases, 64 REQs: Phase 0 (buildWriterOp routing — 12 REQs done), Phase 1 (parser DDL IF EXISTS / RENAME / PRAGMA / TRUNCATE / AUTOINCREMENT — 9 REQs done), Phase 2 (LIMIT/OFFSET, CAST, COALESCE, CASE, ORDER BY expr, HAVING, DISTINCT, constraints, window RANGE, scalar IN — 17 REQs done), Phase 3 (MV-OCC O(1) read-set validation — 1 REQ done), Phase 4 (bugfixes — 20 REQs done + audit REQ000531/532 done), Phase 5 (operator codegen + adaptive compilation, REQ000311/313 shipped — codegen framework + ADQC wrappers), Phase 6 (integration polish — all green). Correlated subquery re-execution fix (store-backed SeqScan). 39/39 packages green with `-race`; 42 done SLT gap tests. | `SQL/PS`, `SQL/EX`, `SQL/PL`, `TXN/MV` | done |

All completed iterations released as v0.9.0–v0.27.2. Iteration 28 done. Coverage details: `go test ./... -cover`.
