...
| 28 | Parser DDL Hardening + SQL Compliance + MV-OCC | 6 phases, 64 REQs: Phase 0 (buildWriterOp routing — 12 REQs done), Phase 1 (parser DDL IF EXISTS / RENAME / PRAGMA / TRUNCATE — 1 REQ done), Phase 2 (LIMIT/OFFSET, CAST, COALESCE, CASE, ORDER BY expr, HAVING, DISTINCT, constraints, window RANGE, scalar IN — partial), Phase 3 (MV-OCC O(1) read-set validation — pending), Phase 4 (bugfixes — 11 REQs done), Phase 5 (operator codegen + adaptive compilation, REQ000311/313, ~4,660 LoC — pending). Audit fixes: REQ000531 ALTER TABLE self-deadlock, REQ000532 Arena.promote() data race. 23 REQs moved from TBD to DONE so far. | `SQL/PS`, `SQL/EX`, `SQL/PL`, `TXN/MV` | in progress (Phase 0 + Phase 4 done; Phase 1/2/3/5 pending) |

All completed iterations released as v0.9.0–v0.27.2. Iteration 28 in progress. Coverage details: `go test ./... -cover`.
