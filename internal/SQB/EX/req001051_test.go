...
func TestParallelIndexRangeScan_EmptyRows(t *testing.T) {
	...
	ps := NewParallelIndexRangeScan(nil, []string{"val"}, []int{0}, "val",
		[]any{int64(1)}, pool)
	defer ps.Close()
...
func TestParallelIndexRangeScan_EmptyValues(t *testing.T) {
	...
	ps := NewParallelIndexRangeScan(rows, []string{"val"}, []int{0}, "val", nil, pool)
	defer ps.Close()
...
func TestParallelIndexRangeScan_NoMatch(t *testing.T) {
	...
	ps := NewParallelIndexRangeScan(rows, []string{"val"}, []int{0}, "val",
...
func BenchmarkParallelIndexRangeScan_100Values(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ps := NewParallelIndexRangeScan(rows, []string{"val"}, []int{0}, "val", values, pool)
		for {
			_, err := ps.Next(ctx)
			if err != nil {
				break
