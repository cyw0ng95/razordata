...
// BenchmarkPlanner_N3WithMemoization measures the cost of N3 join
// ordering on a 7-table join (select4 worst-case shape). With the
// REQ001096 predCache, repeated findPredicatesForSet lookups in the
// inner loop are served from a map instead of O(predicates) scans.
func BenchmarkPlanner_N3WithMemoization(b *testing.B) {
	...
	p := NewPlanner()
	joinTables := make([]joinTableInfo, 6)
	for i := 0; i < 6; i++ {
		joinTables[i] = joinTableInfo{
			name: fmt.Sprintf("t%d", i+2),
		}
	}
	wherePredicates := make([]PS.Expr, 0, 8)
	for i := 0; i < 4; i++ {
		wherePredicates = append(wherePredicates,
			&PS.BinaryExpr{
				...
				Right: &PS.QualifiedName{
					Table: fmt.Sprintf("t%d", i+2),
					Name:  "a",
				},
			})
	}
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		// Run N3 multi-start — exercises the predCache inner loop.
		_ = p.n3JoinOrderingMultiStart("t1", joinTables, wherePredicates, nil)
	}
}

// BenchmarkPlanner_N3SingleStart is the baseline (single base table)
// used to compare against the multi-start memoized version above.
// REQ001096: multi-start cost should approach single-start cost
// when the cache eliminates redundant per-pair predicate lookups.
