...
func TestTBD_RemainingBugfixes(t *testing.T) {
	...
	t.Run("REQ000464 OFFSET", func(t *testing.T) {
		...
		if len(rows) > 0 && rows[0].Data[0] != int64(2) {
	...
	t.Run("REQ000465 EXISTS subquery", func(t *testing.T) {
		...
		if len(rows) != 2 {
			t.Errorf("EXISTS: got %d rows, want 2", len(rows))
		}
	})
	...
	t.Run("REQ000467 GROUP_CONCAT", func(t *testing.T) {
		...
		if len(rows) != 1 {
			t.Errorf("GROUP_CONCAT: got %d rows, want 1", len(rows))
		}
	})
	t.Run("REQ000468 HAVING", func(t *testing.T) {
		...
		if len(rows) != 3 {
			t.Errorf("HAVING: got %d rows, want 3", len(rows))
		}
	})
	...
	t.Run("REQ000470 ORDER BY expr", func(t *testing.T) {
		...
		if len(rows) != 3 {
			t.Errorf("ORDER BY expr: got %d rows, want 3", len(rows))
		}
	})
	t.Run("REQ000471 CAST", func(t *testing.T) {
		...
		if len(rows) != 2 {
			t.Errorf("CAST: got %d rows, want 2", len(rows))
		}
	})
	t.Run("REQ000472 COALESCE", func(t *testing.T) {
		...
		if len(rows) != 1 {
			t.Errorf("COALESCE: got %d rows, want 1", len(rows))
		}
	})
	...
	t.Run("REQ000477 INSERT RETURNING", func(t *testing.T) {
		...
		if len(rows) != 1 {
			t.Errorf("INSERT RETURNING: got %d rows, want 1", len(rows))
		}
	})
	...
	t.Run("REQ000502 scalar subq no dup", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("scalar subq CASE: %v", err)
		}
		if len(rows) != 3 {
			t.Errorf("scalar subq CASE: got %d rows, want 3", len(rows))
		}
	})
	t.Run("REQ000534 VIEW WHERE", func(t *testing.T) {
		...
		if len(rows) != 2 {
			t.Errorf("WHERE x>1: got %d rows, want 2", len(rows))
		}
	})
	t.Run("REQ000533 <> operator", func(t *testing.T) {
		...
		if len(rows) != 2 {
			t.Errorf("<> v!=20: got %d rows, want 2", len(rows))
		}
	})
	t.Run("REQ000535 REPLACE INTO", func(t *testing.T) {
		...
		if len(rows) != 1 {
			t.Fatalf("REPLACE: got %d rows, want 1", len(rows))
		}
	})
}
