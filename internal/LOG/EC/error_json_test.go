...
func TestEC_JSONRoundTrip(t *testing.T) {
	...
	var got Error
	// JSON unmarshal and round-trip validation
...
func TestEC_JSONMarshalSnapshot(t *testing.T) {
	...
	for _, tc := range cases {
		...
		var round Error
		// JSON round-trip integrity check
...
func TestEC_JSONWrapped(t *testing.T) {
	...
	if !strings.Contains(string(data), "inner") {
		// JSON does not contain inner error message
		// Impact: wrapped error information lost in JSON serialization
...
func TestEC_JSONNilWrapped(t *testing.T) {
	e := New(KindNotFound, "simple")
	data, err := json.Marshal(e)
	if err != nil {
		t.Fatalf("MarshalJSON: %v", err)
	}
	// JSON should not contain cause field when no wrapped error
	if strings.Contains(string(data), `"cause"`) {
		t.Error("JSON should not contain cause field when no wrapped error")
	}
}

func TestEC_WireFormatStable(t *testing.T) {
