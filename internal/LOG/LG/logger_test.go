...
// TestSetOutput verifies SetOutput changes the destination writer.
func TestSetOutput(t *testing.T) {
	var buf1 bytes.Buffer
	var buf2 bytes.Buffer
	log := New(Options{Level: slog.LevelDebug, Format: "text", Output: &buf1})
	...
	if buf1.Len() > 0 {
		t.Error("expected no output to buf1 after switch")
	}
	if buf2.Len() == 0 {
		t.Error("expected output to buf2 after switch")
	}
}

// TestSetOutputNil verifies SetOutput handles nil writer.
func TestSetOutputNil(t *testing.T) {
	...
	log.Info("test after nil SetOutput")
}

// TestWithSharedLevel verifies With() shares level with parent.
func TestWithSharedLevel(t *testing.T) {
	var buf bytes.Buffer
	log := New(Options{Level: slog.LevelError, Format: "text", Output: &buf})

	child := log.With("component", "test")

	// Info should be blocked at Error level
	...
	// SetLevel on parent should affect child
	log.SetLevel(slog.LevelInfo)

	log.Info("parent after setlevel")
	if buf.Len() == 0 {
		t.Error("expected output after SetLevel to Info")
	}

	buf.Reset()
	child.Info("child after parent setlevel")
	if buf.Len() == 0 {
		t.Error("expected child output after parent SetLevel")
	}
}

// TestInterfaceComplianceFull verifies logger implements Logger interface with all methods.
